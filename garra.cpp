...
void handleKeypress(unsigned char key, int x, int y) {
	...
	case '6': //Decrease clamp angle y axis
		...
		glutPostRedisplay();
		break;
    case '7': //Move cabeca pra esquerda
		if (angleHead < 70)
            angleH=-0.2;
		glutPostRedisplay();
		break;
      case '8': //Move cabeca pra direita
		if (angleHead < 70)
            angleH=0.2;
		glutPostRedisplay();
		break;
        case '9': //Move cabeca pra posicao inicial
		if (angleHead < 70)
            angleH=0.0;
		glutPostRedisplay();
		break;
...
//    //drawSphere(1.0f);
//    //gluSphere(quadSphere, 1.0, 10.0, 10.0);
//    //drawDisk(diameterCylinder, diameterBase);
//    //gluDisk(quadCylinder, 0.4, 1.5, 40.0, 30.0);
//    drawCylinder(2.0, 1.0);

//    drawCylinder(0.2f, 1.0f);

//        glBegin(GL_QUADS);                                              // Draw A Quad
//                glVertex3f(-1.5f, 1.5f, 0.0f);                          // Top Left
//                glVertex3f( 1.5f, 1.6f, 0.0f);                          // Top Right
//                glVertex3f( 1.5f,-1.6f, 0.0f);                          // Bottom Right
//                glVertex3f(-1.5f,-1.5f, 0.0f);                          // Bottom Left
//        glEnd();
        glRectd(-1.0, -1.4, 1.0, 1.4);
        glPopMatrix();
        drawCylinder(0.3f, 1.1f);
}
void Eyes(){
    glPushMatrix();
